...
set_jsPsych(pavlovia = FALSE, folder = FALSE)は，引数にpavlovia（pavlovia用のRmdファイルを用意するかどうか，デフォルトでは用意しない）とfolder（新たにフォルダを作って，そこに各種ファイルを配置するかどうか，デフォルトでは用意しない）があります。この２つの引数を組み合わせて以下のような活用ができると思います。
